...
assert.match(html, /Explorar 46 fotos/);
assert.match(html, /id="gallery-main-image"/);
assert.match(html, /assets\/photos\/01-sala-02\.webp/);
assert.doesNotMatch(html, /hero-layer-a|carousel-toggle|photo-viewer|>38<|1\/13/);
...
assert.equal(galleryFiles.filter((file) => file.endsWith(".webp")).length, 46, "La galería debe contener 46 imágenes WebP");
assert.equal(thumbnailFiles.filter((file) => file.endsWith(".webp")).length, 46, "La galería debe contener 46 miniaturas WebP");
...
assert.equal(galleryWindow.GALLERY_PHOTOS.length, 46, "La aplicación debe exponer las 46 fotos finales");
assert.equal(galleryWindow.PREVIEW_PHOTOS.length, 46, "La portada dinámica debe recorrer las 46 fotografías");
assert.equal(new Set(galleryWindow.PREVIEW_PHOTOS.map(({ id }) => id)).size, 46, "La portada no debe repetir fotos en su ciclo");
